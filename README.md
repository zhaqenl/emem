...
<dependency>
  ...
  <version>0.2.46-SNAPSHOT</version>
</dependency>
```
...
standalone
one—`./target/uberjar/emem-0.2.46-SNAPSHOT-standalone.jar`. Copy this
file to `~/bin`, as `emem.jar`.

```bash
$ cp target/uberjar/emem-0.2.46-*-standalone.jar ~/bin/emem.jar
```
